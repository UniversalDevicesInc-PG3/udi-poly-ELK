...
When turning on an ELK output you can specify a time of zero which will keep the output on, or set the On Time to the number of seconds to wait before turning off.  You can set the 'Default' time which will be used ever time you turn it on, or you can Turn on With Time for a one time execution.
